...
$resultado = mysqli_query($conexRapiBnB, $consulta) ;
    while ($row = mysqli_fetch_array($resultado)) {
        ...
        if ($variable == '1') {
            while ($record = mysqli_fetch_assoc($resultado2)) {
          ?>
          ...
          <?php
            ...
            $consulta3="SELECT * FROM registroreserva WHERE idusuario = '$idusuario' ";
            $resultado3 = mysqli_query($conexRapiBnB, $consulta3) ;

            while ($record = mysqli_fetch_assoc($resultado3)) {
                $idalojamiento=$record['idalojamiento'];

                $alojamiento="SELECT * FROM registroalojamiento WHERE ID = '$idalojamiento'";
                ?>
                <div class="row row-cols1 row-cols-sm-2 row-cols-md-3 g-3" id="card">
                <div class="col" >
                <div class="card shadow sm" id="card">
